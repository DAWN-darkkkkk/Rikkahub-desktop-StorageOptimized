...
const stateBackupPath = join(dataDir, "state.json.backup");
...
const LOG_PREVIEW_LIMIT = 32_000;
...
function normalizeState(input: Partial<State>): State {
  ...
  const normalized: State = {
    ...
    conversationIndices: Array.isArray(input.conversationIndices) ? input.conversationIndices : [],
...
function loadState(): State {
  ...
  if (schemaVersion >= 2) {
    // 新格式：state.json 不含对话消息体，从独立文件加载对话
    const state = normalizeState(raw as Partial<State>);
    // 迁移：删除日志中冗余的 requestBody/responseBody（已由 smart-truncated requestPreview/responsePreview 替代）
    if (Array.isArray(state.logs)) {
      for (const log of state.logs) {
        delete (log as Record<string, unknown>).requestBody;
        delete (log as Record<string, unknown>).responseBody;
      }
    }
    state.conversations = loadAllConversationsFromFiles();
    // 以文件系统实际存在为准重建索引
    state.conversationIndices = state.conversations.map(toConversationIndex);
    return state;
  }
  // 旧格式迁移：state.json 内含完整对话消息体
  const state = normalizeState(raw as Partial<State>);
  // 迁移：删除日志中冗余的 requestBody/responseBody
  if (Array.isArray(state.logs)) {
    for (const log of state.logs) {
      delete (log as Record<string, unknown>).requestBody;
      delete (log as Record<string, unknown>).responseBody;
  ...
  try {
    copyFileSync(statePath, stateBackupPath);
  } catch {
    console.warn("Failed to backup old state.json before migration");
  }
  migrateConversationsToFiles(state.conversations);
  state.schemaVersion = 2;
  state.conversationIndices = state.conversations.map(toConversationIndex);
  // 写一次精简后的 state.json（仅含索引不含对话体）
  const diskState: Record<string, unknown> = {};
  for (const key of Object.keys(state)) {
    if (key === "conversations") continue;
    (diskState as Record<string, unknown>)[key] = (state as Record<string, unknown>)[key];
  }
  diskState.conversationIndices = state.conversationIndices;
  diskState.schemaVersion = 2;
  mkdirSync(dataDir, { recursive: true });
  writeFileSync(statePath, JSON.stringify(diskState, null, 2));
  return state;
}
...
function scheduleThrottledSaveState(conversation?: Conversation) {
  // 流式传输中：仅保存当前会话文件，不写 state.json
  if (conversation) {
    scheduleThrottledSaveConversation(conversation);
    return;
  }
  // 非流式调用：写 state.json（保持原有行为）
  const now = Date.now();
...
let ftsDb: Database | null = null;

// 初始化全文搜索索引
initFts();
// 如果 FTS 为空表或指定 --reindex-fts 参数，则重建全部索引
if (ftsDb) {
  const row = ftsDb.query("SELECT COUNT(*) as cnt FROM conversation_fts").get() as { cnt: number } | null;
  if (!row || row.cnt === 0 || process.argv.includes("--reindex-fts")) rebuildAllFtsIndex();
}
...
async function performStateSave(): Promise<void> {
  ...
  mkdirSync(dataDir, { recursive: true });
  // 构建磁盘状态：仅保存对话索引以替代完整的对话体
  const diskState: Record<string, unknown> = {};
  for (const key of Object.keys(state)) {
    if (key === "conversations") continue;
    (diskState as Record<string, unknown>)[key] = (state as Record<string, unknown>)[key];
  }
  diskState.conversationIndices = state.conversations.map(toConversationIndex);
  diskState.schemaVersion = 2;
  const content = JSON.stringify(diskState, null, 2);
...
function writeConversationFile(conversation: Conversation): void {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    try {
      writeFileSync(tempPath, JSON.stringify(conversation, null, 2));
  ...
  try {
    ...
    console.warn("Failed to write conversation file", absPath, lastError ?? err);
  }
}

function readConversationFile(filePath: string): Conversation | null {
  try {
    const absPath = join(conversationsDir, filePath);
...
function deleteConversationFile(filePath: string): void {
  try {
    const absPath = join(conversationsDir, filePath);
    if (existsSync(absPath)) unlinkSync(absPath);
  } catch {
    console.warn("Failed to delete conversation file", filePath);
  }
}

function loadAllConversationsFromFiles(): Conversation[] {
  ...
  for (const year of yearDirs) {
    ...
    for (const dateDir of dateDirs) {
      ...
      for (const file of files) {
        ...
        const conv = readConversationFile(join(year, dateDir, file));
        if (conv) results.push(conv);
      }
    }
  }
  results.sort((a, b) => b.updateAt - a.updateAt);
  return results;
}

function migrateConversationsToFiles(conversations: Conversation[]): void {
  for (const conv of conversations) {
    writeConversationFile(conv);
  }
}
...
function initFts(): void {
  try {
    ...
    ftsDb.exec("PRAGMA journal_mode=WAL");
    ftsDb.exec(`
    ...
    console.warn("Failed to initialize FTS database", err);
    ftsDb = null;
  }
}

function closeFts(): void {
  if (ftsDb) {
...
function extractConversationText(conversation: Conversation): string {
  const parts: string[] = [];
  for (const node of conversation.messages) {
    const msg = node.messages[node.selectIndex];
    if (msg) {
      parts.push(textFromParts(msg.parts));
    }
  }
  return parts.join("\n");
}

function indexConversation(conversation: Conversation): void {
  if (!ftsDb) return;
  try {
    ftsDb.exec("DELETE FROM conversation_fts WHERE conversation_id = ?", [conversation.id]);
    const text = extractConversationText(conversation);
    ftsDb.exec("INSERT INTO conversation_fts VALUES (?, ?, ?)", [
      conversation.id,
      conversation.title,
      text,
    ]);
  } catch {
    // FTS indexing errors must not surface to the user
  }
}

function deindexConversation(conversationId: string): void {
  if (!ftsDb) return;
  try {
    ftsDb.exec("DELETE FROM conversation_fts WHERE conversation_id = ?", [conversationId]);
  } catch { /* ignore */ }
}

function searchConversationsFts(
  query: string,
  assistantId?: string,
): { nodeId: string; messageId: string; conversationId: string; title: string; updateAt: number; snippet: string }[] {
  if (!ftsDb || !query.trim()) return [];
  try {
    // 使用 FTS5 前缀搜索：在每个词尾追加 '*'
...
function rebuildAllFtsIndex(): void {
  if (!ftsDb) return;
  try {
    ftsDb.exec("DELETE FROM conversation_fts");
...
function saveConversationFile(conversation: Conversation): void {
  // 同步 state 中的索引条目
  const idx = state.conversationIndices.find((item) => item.id === conversation.id);
  if (idx) {
    idx.title = conversation.title;
    idx.updateAt = conversation.updateAt;
    idx.messageCount = conversation.messages.length;
    idx.isPinned = conversation.isPinned;
  } else {
    state.conversationIndices.push(toConversationIndex(conversation));
  }
  writeConversationFile(conversation);
  indexConversation(conversation);
}

let pendingThrottledConvSaveId: string | null = null;
let pendingThrottledConvSaveTimer: ReturnType<typeof setTimeout> | null = null;
let lastConvSaveMs = 0;

function scheduleThrottledSaveConversation(conversation: Conversation): void {
  pendingThrottledConvSaveId = conversation.id;
  const now = Date.now();
  const elapsed = now - lastConvSaveMs;
  if (elapsed >= STREAM_SAVE_INTERVAL_MS) {
    if (pendingThrottledConvSaveTimer) {
      clearTimeout(pendingThrottledConvSaveTimer);
      pendingThrottledConvSaveTimer = null;
    }
    saveConversationFile(conversation);
    return;
  }
  if (pendingThrottledConvSaveTimer) return;
  pendingThrottledConvSaveTimer = setTimeout(() => {
    pendingThrottledConvSaveTimer = null;
    const conv = pendingThrottledConvSaveId ? getConversation(pendingThrottledConvSaveId) : null;
    if (conv) saveConversationFile(conv);
    pendingThrottledConvSaveId = null;
  }, STREAM_SAVE_INTERVAL_MS - elapsed);
}
...
function deleteConversationsById(ids: Set<string>) {
  ...
  for (const conv of state.conversations) {
    if (ids.has(conv.id)) {
      const idx = state.conversationIndices.find((item) => item.id === conv.id);
      if (idx) deleteConversationFile(idx.filePath);
      deindexConversation(conv.id);
    }
  }
  state.conversationIndices = state.conversationIndices.filter((item) => !ids.has(item.id));
...
function jsonPreview(value: unknown) {
  return JSON.stringify(smartTruncate(value), null, 2);
}

function textPreview(value: string, limit = 100) {
...
function applyPcBackupFromExtractDir(extractDir: string, pcBackupPath: string): { settingsImported: boolean; filesImported: number; skillsImported: number; conversationsImported: number; dbReadError: string | null } {
  ...
  try {
    ...
    // 恢复 conversations/ 目录中的会话文件（新格式备份）
    const convDir = join(extractDir, "conversations");
    if (existsSync(convDir) && !conversationsImported) {
      copyDirRecursive(convDir, conversationsDir);
      const fileConvs = loadAllConversationsFromFiles();
      if (fileConvs.length > 0) {
        const existing = new Map(state.conversations.map((c) => [c.id, c]));
        for (const fc of fileConvs) {
          existing.set(fc.id, fc);
        }
        state.conversations = Array.from(existing.values()).sort((a, b) => b.updateAt - a.updateAt);
        conversationsImported = fileConvs.length;
      }
    }
    // 将恢复的会话写入独立文件（导入自 DB 的已在 importAndroidConversations 中写入）
    for (const conv of state.conversations) {
      const convIdx = state.conversationIndices.find((item) => item.id === conv.id);
      if (!convIdx) saveConversationFile(conv);
    }
    saveState();
    broadcastSettings();
    broadcastList();
...
function createSettingsBackupZipToPath(targetZipPath: string, onProgress?: (message: string) => void): number {
  ...
  try {
    ...
    // 包含会话文件目录（新格式）
    if (existsSync(conversationsDir) && readdirSync(conversationsDir).length > 0) {
      onProgress?.("正在打包会话记录...");
      const convStage = join(stageDir, "conversations");
      mkdirSync(convStage, { recursive: true });
      copyDirRecursive(conversationsDir, convStage);
    }
    onProgress?.("正在压缩...");
...
function touchStream(hooks?: StreamHooks) {
  ...
  scheduleThrottledSaveState(hooks.conversation);
...
async function routeApi(request: Request, url: URL) {
  ...
  if (path === "conversations/search" && request.method === "GET") {
    ...
    if (!queryText) return json([]);
    // 优先走 FTS 全文搜索
    const ftsResults = searchConversationsFts(queryText, state.settings.assistantId);
    if (ftsResults.length > 0) return json(ftsResults);
    // FTS 不可用或未命中时回退到线性扫描
